...
def organization_created_handler(sender, instance, created, **kwargs):
    ...
    if created and ACTIVITY_LOGS_ENABLED:
        ActivityLog.objects.create(
            ...
            description=f"Organization '{instance.name}' was created"
        )
...
def organization_member_handler(sender, instance, created, **kwargs):
    ...
    if created and ACTIVITY_LOGS_ENABLED:
        ActivityLog.objects.create(
            ...
            description=f"{instance.user.get_full_name()} joined organization '{instance.organization.name}' with role {instance.get_role_display()}"
        )
    ...
    if ACTIVITY_LOGS_ENABLED:
        if created:
            ActivityLog.objects.create(
                ...
                object_id=str(instance.organization.id),
                action_type=ActivityLog.UPDATED,
                description=f"Invitation sent to {instance.email} for organization '{instance.organization.name}'"
            )
            ...
            ActivityLog.objects.create(
                ...
                object_id=str(instance.organization.id),
                action_type=ActivityLog.UPDATED,
                description=f"Invitation to {instance.email} for organization '{instance.organization.name}' was accepted"
            ) 